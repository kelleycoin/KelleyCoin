...
int64 nChainStartTime = 1465574400;
...
const string strMessageMagic = "KelleyCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // KelleyCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
     ...
     if(nHeight == 1)
     {
       nSubsidy = 390000000 * COIN;	// 390 million coins, that all pow coins
...
const int YEARLY_BLOCKCOUNT = 365 * 1440;
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    if(nHeight < YEARLY_BLOCKCOUNT * 5)              //3%
        nSubsidy *= 10;
    else
        nSubsidy = 0;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in KelleyCoin
    bool fStrictPayToScriptHash = true; // Always active in KelleyCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "KelleyCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "KelleyCoin is upcoming release. Wish is a great coin! Jun 11 2016.";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0x6294d892c9ca930ec563d6c1eae882159d67e47633963e16d5622431fb7a1711"));
...
unsigned char pchMessageStart[4] = { 0x00, 0x28, 0x06, 0x11 };
